...
import { eventEmit } from "../core/events.mjs";
    ...
    ws.on('message', async function incoming(data, isBinary) {
        ...
        switch (parsedData.op) {
            ...
            case 11:
                logger.log({
                    level: 'debug',
                    message: "DBG || ws.on('message') || REC OPCODE 11 HEARTBEAT ACK"
                });
                break;
        ...
        switch (parsedData.t) {
            ...
            case "READY":
                ...
                eventEmit('ready', parsedData.d);
                break;

            case "MESSAGE_CREATE":
                logger.log({
                    level: 'debug',
                    message: "DBG || ws.on('message') || REC MESSAGE_CREATE EVENT || Data: " + JSON.stringify(parsedData)
                });
                eventEmit('message', parsedData.d);
                break;
        ...
        function heartbeat() {
            ...
            logger.log({
                level: 'debug',
                message: "DBG || HEARTBEAT || SENT OPCODE 1"
            });
        };
    });
}